...
import { ArrowRight, Sparkles, Heart, Building2, Shield, Home, Users, FileText, Gavel, Briefcase, Icon } from 'lucide-react';
...
const CategoriesSection = ({ onCategorySelect, onViewAll }) => {
  ...
  const iconMap : Record<string, React.ElementType>={
    ...
    useEffect(()=>{
      ...
      .then(data=>{
        ...
        setFetchedCategories(mappedData);
        
        console.log("Fetched Categories:", JSON.stringify(mappedData));
      }).catch(error=>{
        throw error;
      })
    },[])
  ...
  const displayedCategories = fetchedCategories.slice(0, 6);

  return (
    <section id="practice-areas" className="py-20 bg-gradient-to-br from-slate-50 via-blue-50/30 to-purple-50/30 relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center">
          <button
            onClick={() => onViewAll(fetchedCategories)}
          ...
          <p className="text-slate-500 mt-6 text-sm">
            Explore all {fetchedCategories.length} legal practice areas available on our platform
          </p>
        </div>
      </div>
    </section>
